$(function() {
            ...
            <tr>
                <td>
                    <select class="form-control yarn-count-select">
                        ...
                        ${yarnCounts.map(yc => `<option value="${yc.id}">${yc.name}</option>`).join('')}
                    </select>
                </td>
    ...
    function updateTotals() {
        ...
        if (hasItems) {
            ...
            const addedItemsHtml = $('#itemsTable tbody tr').map(function() {
                const yarnCountId = $(this).find('.yarn-count-select').val();
                if (!yarnCountId) return '';
                
                const yarnCountName = $(this).find('.yarn-count-select option:selected').text();
                const quantity = parseFloat($(this).find('.quantity-input').val()) || 0;
                const unitPrice = parseFloat($(this).find('.unit-price-input').val()) || 0;
